...
// 回復使用者資料
function Recover_Member(string $_id)
{
    $link = DB_Connect(); // 執行連線函式
    $UpdateResult = false;
    ...
    try {
        if ($link === false) {
            ...
            $sql = "UPDATE users SET del = 0 WHERE id = ?";
...
// 刪除或回復使用者資料
function Grant_Member(string $_id, string $_isDelete)
{
    $link = DB_Connect(); // 執行連線函式
    $UpdateResult = false;
    ...
    try {
        if ($link === false) {
            ...
            $sql = "UPDATE users SET del = ? WHERE id = ?";

            echo "<p>SQL查詢字串: $sql </p>";

            if ($stmt = mysqli_prepare($link, $sql)) {
                mysqli_stmt_bind_param($stmt, "ss", $param_isDelete, $param_id);

                // 設定參數值
                $param_id = $_id;
                $param_isDelete = $_isDelete;

...
<body>
    ...
    <table border="1">
        ...
        foreach ($temp as $row) { // 依序讀取每一行
            ...
            echo "<td><a href = 'grant_member.php?id={$row["id"]}&isDelete=1'>刪除</a></td>";
            ...
            echo "<td>{$row["del"]}</td>";
            if ($row["del"] == "1")
                // echo "<td><a href = 'recover_account.php?id={$row["id"]}'>回復</a></td>";
                echo "<td><a href = 'grant_member.php?id={$row["id"]}&isDelete=0'>回復</a></td>";
            else
                echo "<td></td>";
            echo "</tr>";
        }

...
if (isset($_GET["function"]) && $_GET["function"] != "") {
    ...
    switch ($function) {
        case 'Query_All_Students':
            $QueryResults = Query_All_Students();
            break;
        case 'Query_One_Student':
            if (isset($_GET["id"]))
                $QueryResults = Query_One_Student($_GET["id"]);
            break;
        case 'Update_One_Student':
            if (isset($_POST["name"]) && isset($_POST["gender"]) && isset($_POST["id"])) {
                $QueryResults = Update_One_Student($_POST["name"], $_POST["gender"], $_POST["id"]);
            ...
            break;
        case 'Insert_One_Student':
            if (isset($_POST["name"]) && isset($_POST["gender"])) {
                $QueryResults = Insert_One_Student($_POST["name"], $_POST["gender"]);
                if ($QueryResults == true)
                    echo "新增成功";
                else
                    echo "新增失敗";
        ...
        case 'Delete_One_Student':
            if (isset($_GET["id"])) {
                $QueryResults = Delete_One_Student($_GET["id"]);
                if ($QueryResults == true)
                    echo "刪除成功";
                else
                    echo "刪除失敗";
...
function Query_All_Students()
{
    ...
    try {
        if ($pdo === false) {
            ...
            $sql = "SELECT id, name, gender FROM students WHERE del = 0";

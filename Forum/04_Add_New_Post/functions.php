<?php
function DB_Connect()
{
    // 資料庫連線基本設定
    $dbms = 'mysql';     //資料庫類型
    $host = 'localhost'; //資料庫位址
    $dbName = 'forum';   //預設資料庫
    $user = 'root';      //帳號
    $pass = '';          //密碼
    $dsn = "$dbms:host=$host;dbname=$dbName";

    try {
        $pdo = new PDO($dsn, $user, $pass); // 建立資料庫連線
        $pdo->exec('SET CHARACTER SET utf8mb4');

        if ($pdo === false) {
            die("發生錯誤無法連線");
            exit;
        }
    } catch (PDOException $e) {
        echo 'Caught exception: ',  $e->getMessage(), "\n";
    }

    return $pdo;
}

// 帳號密碼確認
function Account_Check(string $_account, string $_password)
{
    //讀取帳號記錄的檔案
    $result = null; // $result 變數用來做回傳值
    $pdo = DB_Connect(); // 連線函式

    try {
        if ($pdo === false) {
            die("發生錯誤無法連線");
        } else {
            echo "<p>資料庫連線成功</p>";
            // SQL查詢指令，請特別注意：SELECT 只抓取五個欄位值 id, account, password, name, admin，這個順序會和以下的查詢結果相對應，別弄混
            $sql = "SELECT id, account, password, name, admin FROM users WHERE account = :account AND password = :password";
            $statement = $pdo->prepare($sql);
            // 綁定變數值
            $statement->bindParam(':account', $_account, PDO::PARAM_STR);   
            $statement->bindParam(':password', $_password, PDO::PARAM_STR);

            // 判斷查詢指令有沒有成功
            if ($statement->execute()) {
                $result = $statement->fetch(PDO::FETCH_ASSOC); // 將查詢結果存到 $result
            }
        }
    } catch (PDOException $e) {
        echo '資料庫錯誤: ',  $e->getMessage(), "\n";
    } finally {
        unset($PDO);
    }

    return $result;
}

// 依照使用者的權限分流到不同頁面
function Check_Member_Authority()
{    
    if (isset($_SESSION["admin"])) {
        echo "<p>你過去已經登入過，5秒後頁面將自動跳轉到你的專屬頁面！</p>";
        if ($_SESSION["admin"] == "Y") {
            header("refresh:5; url=admin.php"); // 如果SESSION['admin']是Y，代表使用者有管理者權限，導引到管理者頁面
        } else {
            header("refresh:5; url=member.php?id={$_SESSION["id"]}"); // 一般使用者，導引到一般會員頁面
        }
    }
}

// 查詢所有使用者資料
function Query_All_Members()
{
    $result = null; // $result 變數用來做回傳值
    $pdo = DB_Connect(); // 連線函式

    try {
        if ($pdo === false) {
            die("發生錯誤無法連線");
        } else {
            echo "<p>資料庫連線成功</p>";
            $sql = "SELECT id, account, password, name, admin FROM users";
            $statement = $pdo->prepare($sql);

            // 判斷查詢指令有沒有成功
            if ($statement->execute()) {
                $result = $statement->fetchAll(PDO::FETCH_ASSOC); // 將查詢結果存到 $result
            }
        }
    } catch (PDOException $e) {
        echo '資料庫錯誤: ',  $e->getMessage(), "\n";
    } finally {
        unset($PDO);
    }

    return $result;
}

// 查詢單一使用者
function Query_One_Member(string $_id)
{
    $result = null; // $result 變數用來做回傳值
    $pdo = DB_Connect(); // 連線函式

    try {
        if ($pdo === false) {
            die("發生錯誤無法連線");
        } else {
            echo "<p>資料庫連線成功</p>";
            $sql = "SELECT id, account, password, name, admin FROM users WHERE id = :id";
            $statement = $pdo->prepare($sql);
            // 綁定變數值
            $statement->bindParam(':id', $_id, PDO::PARAM_STR);   

            // 判斷查詢指令有沒有成功
            if ($statement->execute()) {
                $result = $statement->fetch(PDO::FETCH_ASSOC); // 將查詢結果存到 $result
            }
        }
    } catch (PDOException $e) {
        echo '資料庫錯誤: ',  $e->getMessage(), "\n";
    } finally {
        unset($PDO);
    }

    return $result;
}

// 更新使用者資料
function Update_Member(string $_account, string $_password, string $_name, string $_admin, string $_id)
{
    $result = false; // $result 變數用來做回傳值
    $pdo = DB_Connect(); // 連線函式

    try {
        if ($pdo === false) {
            die("發生錯誤無法連線");
        } else {
            echo "<p>資料庫連線成功</p>";
            $sql = "UPDATE users SET account = :account,
                                     password = :password,
                                     name = :name,
                                     admin = :admin
                    WHERE id = :id";

            $statement = $pdo->prepare($sql);
            // 綁定變數值
            $statement->bindParam(':account', $_account, PDO::PARAM_STR); 
            $statement->bindParam(':password', $_password, PDO::PARAM_STR); 
            $statement->bindParam(':name', $_name, PDO::PARAM_STR); 
            $statement->bindParam(':admin', $_admin, PDO::PARAM_STR); 
            $statement->bindParam(':id', $_id, PDO::PARAM_STR);   

            // 判斷查詢指令有沒有成功
            if ($statement->execute()) {
                $result = true; // 將查詢結果存到 $result
            }
        }
    } catch (PDOException $e) {
        echo '資料庫錯誤: ',  $e->getMessage(), "\n";
    } finally {
        unset($PDO);
    }

    return $result;
}

// 新增使用者資料
function Add_Member(string $_account, string $_password, string $_name, string $_admin)
{
    $result = false; // $result 變數用來做回傳值
    $pdo = DB_Connect(); // 連線函式

    try {
        if ($pdo === false) {
            die("發生錯誤無法連線");
        } else {
            echo "<p>資料庫連線成功</p>";
            $sql = "INSERT INTO users (account, password, name, admin) 
                    VALUES (:account, :password, :name, :admin)";

            $statement = $pdo->prepare($sql);
            // 綁定變數值
            $statement->bindParam(':account', $_account, PDO::PARAM_STR); 
            $statement->bindParam(':password', $_password, PDO::PARAM_STR); 
            $statement->bindParam(':name', $_name, PDO::PARAM_STR); 
            $statement->bindParam(':admin', $_admin, PDO::PARAM_STR);  

            // 判斷查詢指令有沒有成功
            if ($statement->execute()) {
                $result = true; // 將查詢結果存到 $result
            }
        }
    } catch (PDOException $e) {
        echo '資料庫錯誤: ',  $e->getMessage(), "\n";
    } finally {
        unset($PDO);
    }

    return $result;
}

// 刪除使用者資料
function Delete_Member(string $_id)
{
    $result = false; // $result 變數用來做回傳值
    $pdo = DB_Connect(); // 連線函式

    try {
        if ($pdo === false) {
            die("發生錯誤無法連線");
        } else {
            echo "<p>資料庫連線成功</p>";
            $sql = "DELETE FROM users WHERE id = :id";

            $statement = $pdo->prepare($sql);
            // 綁定變數值
            $statement->bindParam(':id', $_id, PDO::PARAM_STR); 

            // 判斷查詢指令有沒有成功
            if ($statement->execute()) {
                $result = true; // 將查詢結果存到 $result
            }
        }
    } catch (PDOException $e) {
        echo '資料庫錯誤: ',  $e->getMessage(), "\n";
    } finally {
        unset($PDO);
    }

    return $result;
}

// 查詢所有文章標題
function Query_All_Post_Titles()
{
    $result = null; // $result 變數用來做回傳值
    $pdo = DB_Connect(); // 連線函式

    try {
        if ($pdo === false) {
            die("發生錯誤無法連線");
        } else {
            echo "<p>資料庫連線成功</p>";
            // 結合使用者資料與文章資料表格，SQL指令可以同時結合多個表格，讓你整合資料，務必學會！！
            $sql = "SELECT posts.*, users.name FROM posts, users WHERE posts.user_id = users.id";
            $statement = $pdo->prepare($sql);

            // 判斷查詢指令有沒有成功
            if ($statement->execute()) {
                $result = $statement->fetchAll(PDO::FETCH_ASSOC); // 將查詢結果存到 $result
            }
        }
    } catch (PDOException $e) {
        echo '資料庫錯誤: ',  $e->getMessage(), "\n";
    } finally {
        unset($PDO);
    }

    return $result;
}

// 查詢特定文章的回文
function Query_One_Post_Details(string $_post_id)
{
    $result = null; // $result 變數用來做回傳值
    $pdo = DB_Connect(); // 連線函式

    try {
        if ($pdo === false) {
            die("發生錯誤無法連線");
        } else {
            echo "<p>資料庫連線成功</p>";
            $sql = "SELECT post_detail.*, users.name FROM post_detail, users WHERE post_detail.user_id = users.id AND post_detail.post_id = :post_id";

            $statement = $pdo->prepare($sql);
            // 綁定變數值
            $statement->bindParam(':post_id', $_post_id, PDO::PARAM_STR); 

            // 判斷查詢指令有沒有成功
            if ($statement->execute()) {
                $result = $statement->fetchAll(PDO::FETCH_ASSOC); // 將查詢結果存到 $result
            }
        }
    } catch (PDOException $e) {
        echo '資料庫錯誤: ',  $e->getMessage(), "\n";
    } finally {
        unset($PDO);
    }

    return $result;
}

// 查詢特定文章標題
function Query_One_Post_Title(string $_post_id)
{
    $result = null; // $result 變數用來做回傳值
    $pdo = DB_Connect(); // 連線函式

    try {
        if ($pdo === false) {
            die("發生錯誤無法連線");
        } else {
            echo "<p>資料庫連線成功</p>";
            $sql = "SELECT title FROM posts WHERE id = :post_id"; // 只會查出某個特定文章的標題

            $statement = $pdo->prepare($sql);
            // 綁定變數值
            $statement->bindParam(':post_id', $_post_id, PDO::PARAM_STR); 

            // 判斷查詢指令有沒有成功
            if ($statement->execute()) {
                $result = $statement->fetch(PDO::FETCH_ASSOC); // 將查詢結果存到 $result
            }
        }
    } catch (PDOException $e) {
        echo '資料庫錯誤: ',  $e->getMessage(), "\n";
    } finally {
        unset($PDO);
    }

    return $result;
}

// 新增回文
function Add_Post_Detail(string $_post_id, string $_user_id, string $_message)
{
    $result = false; // $result 變數用來做回傳值
    $pdo = DB_Connect(); // 連線函式

    try {
        if ($pdo === false) {
            die("發生錯誤無法連線");
        } else {
            echo "<p>資料庫連線成功</p>";
            $sql = "INSERT INTO post_detail (post_id, user_id, message) VALUES (:post_id, :user_id, :message)";

            $statement = $pdo->prepare($sql);
            // 綁定變數值
            $statement->bindParam(':post_id', $_post_id, PDO::PARAM_STR); 
            $statement->bindParam(':user_id', $_user_id, PDO::PARAM_STR); 
            $statement->bindParam(':message', $_message, PDO::PARAM_STR); 

            // 判斷查詢指令有沒有成功
            if ($statement->execute()) {
                $result = true; // 將查詢結果存到 $result
            }
          
        }
    } catch (PDOException $e) {
        echo '資料庫錯誤: ',  $e->getMessage(), "\n";
    } finally {
        unset($PDO);
    }

    return $result;
}

// 新增主題文章
// 特點：連續執行好幾個SQL指令，但這不是一個最好的寫法，假設其中一個SQL指令出問題，就無法回復或者取消執行，會造成資料異常
// 實務解決方法：要使用「交易Transaction」的作法來對資料庫作存取
function Add_New_Post(string $_title, string $_user_id, string $_message)
{
    $result = false; // $result 變數用來做回傳值
    $pdo = DB_Connect(); // 連線函式    
    $_post_id = "";
    $tempResults = array(false, false); // 陣列變數tempResults用來記錄新增兩個資料表是否成功

    try {
        if ($pdo === false) {
            die("發生錯誤無法連線");
        } else {
            echo "<p>資料庫連線成功</p>";

            // 新增文章標題
            $sql = "INSERT INTO posts (user_id, title) VALUES (:user_id, :title)";

            $statement = $pdo->prepare($sql);
            // 綁定變數值
            $statement->bindParam(':user_id', $_user_id, PDO::PARAM_STR); 
            $statement->bindParam(':title', $_title, PDO::PARAM_STR); 

            // 判斷新增文章標題有沒有成功
            if ($statement->execute()) {
                $tempResults[0] = true; // 新增文章標題成功

                // 取得最新的文章標題id
                $_post_id = $pdo->lastInsertId();
            }

            // 新增回文
            $sql = "INSERT INTO post_detail (post_id, user_id, message) VALUES (:post_id, :user_id, :message)";

            $statement = $pdo->prepare($sql);
            // 綁定變數值
            $statement->bindParam(':post_id', $_post_id, PDO::PARAM_STR); 
            $statement->bindParam(':user_id', $_user_id, PDO::PARAM_STR); 
            $statement->bindParam(':message', $_message, PDO::PARAM_STR); 

            // 判斷新增回文有沒有成功
            if ($statement->execute()) {
                $tempResults[1] = true; // 新增回文成功
            }

            // 判斷陣列變數tempResults兩個值是否都是true，是的話才回傳成功的訊息，但這不是好作法
            if ($tempResults[0] && $tempResults[1]) {
                $result = true;
            }             
        }
    } catch (PDOException $e) {
        echo '資料庫錯誤: ',  $e->getMessage(), "\n";
    } finally {
        unset($PDO);;
    }

    return $result; // $AddResult 變數回傳是否有成功回文
}

// 新增主題文章，使用「交易Transaction」的作法來對資料庫作存取
function Add_New_Post_Trans(string $_title, string $_user_id, string $_message)
{
    $result = false; // $result 變數用來做回傳值
    $pdo = DB_Connect(); // 連線函式    
    $_post_id = "";
    $tempResults = array(false, false); // 陣列變數tempResults用來記錄新增兩個資料表是否成功

    try {
        if ($pdo === false) {
            die("發生錯誤無法連線");
        } else {
            echo "<p>資料庫連線成功</p>";
            // 開啟一個交易
             $pdo->beginTransaction();

            // 新增文章標題
            $sql = "INSERT INTO posts (user_id, title) VALUES (:user_id, :title)";

             $statement = $pdo->prepare($sql);
            // 綁定變數值
            $statement->bindParam(':user_id', $_user_id, PDO::PARAM_STR); 
            $statement->bindParam(':title', $_title, PDO::PARAM_STR); 

            // 判斷新增文章標題有沒有成功
            if ($statement->execute()) {
                $tempResults[0] = true; // 新增文章標題成功

                // 取得最新的文章標題id
                $_post_id = $pdo->lastInsertId();
            }

            // 新增回文
            $sql = "INSERT INTO post_detail (post_id, user_id, message) VALUES (:post_id, :user_id, :message)";

            $statement = $pdo->prepare($sql);
            // 綁定變數值
            $statement->bindParam(':post_id', $_post_id, PDO::PARAM_STR); 
            $statement->bindParam(':user_id', $_user_id, PDO::PARAM_STR); 
            $statement->bindParam(':message', $_message, PDO::PARAM_STR); 

            // 判斷新增回文有沒有成功
            if ($statement->execute()) {
                $tempResults[1] = true; // 新增回文成功
            }

            // 判斷陣列變數tempResults兩個值是否都是true，是的話才回傳成功的訊息
            if ($tempResults[0] && $tempResults[1]) {
                $pdo->commit(); // 如果資料都成功新增，進行commit
                $result = true;
            } else {
                $pdo->rollBack(); // 否則rollback
            }                
        }
    } catch (PDOException $e) {
        if ($pdo->inTransaction()) {
            $pdo->rollBack(); // 如果發生錯誤，取消資料庫資料的更動
        }
        echo '資料庫錯誤: ',  $e->getMessage(), "\n";
    } finally {
        unset($PDO);;
    }

    return $result; // $AddResult 變數回傳是否有成功回文
}
